...
@asset(
  # <-- SUPPRIMEZ cette ligne et ajoutez le paramètre typé raw_customer_data: pd.DataFrame à la fonction
    description="Features préparées pour l'entraînement",
    compute_kind="pandas"
)
def preprocessed_features(raw_customer_data: pd.DataFrame) -> pd.DataFrame:  # <-- AJOUTEZ raw_customer_data: pd.DataFrame
    ...
    # Exemple de traitement simple : normalisation des colonnes numériques
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    for col in numeric_cols:
        df[f"{col}_normalized"] = (df[col] - df[col].mean()) / (df[col].std() + 1e-8)
    return df  # Retourne les données prétraitées
...
@asset(
    description="Modèle entraîné avec des métadonnées affichées dans l'UI",
...
@asset(
    description="Modèle entraîné avec tracking MLflow",
    compute_kind="sklearn"
)
